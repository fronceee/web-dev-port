const jobPlaces = [
  { 
    place: 'PEAK Account',
    title: 'Junior Frontend Developer',
    time: 'Nov 2023 - Present',
    desc: [
      'Assist the team to deliver quality tech/coding online courses on the platform',
      'Research and developing tech course markets for current courses and possible offline and online courses',
      "Answer learners' questions about content in tech courses via the platform's discussion boards and other channels and coordinating more technical questions to instructors.",
    ]
  },
  { 
    place: 'Skooldio',
    title: 'Course Development Associate (Tech/Coding)',
    time: 'July 2022 - October 2023',
    desc: [
      'Assist the team to deliver quality tech/coding online courses on the platform',
      'Research and developing tech course markets for current courses and possible offline and online courses',
      "Answer learners' questions about content in tech courses via the platform's discussion boards and other channels and coordinating more technical questions to instructors.",
    ]
  },

  { 
    place: 'Chulalongkorn University Demonstration\nSecondary School',
    title: 'Student Teacher',
    time: 'May 2022 - Oct 2022',
    desc: [
      'Responsible for teaching, lesson planning, and creating learning material for M2 English Foundation classes',
      'Responsible for teaching, lesson planning, and creating learning material for M2 Computing Science classes',
      'Teaching and delivering both subjects in online formats.'
    ]
  },
  { 
    place: 'Algorithmics BangkokMetro',
    title: 'Part-Time Python Teacher',
    time: 'Dec 2020 - Present',
    desc: [
      'Teaching Python for kids aged 12-15 who have no prior coding background to be able to create simple coding projects',
      'Teaching Python from basic syntax to complex concepts in programming such as OOP and Dictionary.',
      'Adapting pre-planned lessons to be suitable for learners in each class.',
    ]
  }
]

export { jobPlaces }